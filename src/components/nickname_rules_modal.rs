// Modal component for viewing and managing vendor nickname rules.
// Allows user to select rules, delete rules, and edit rule matching regexes and nicknames.

use crate::components::nickname_editor_modal::NicknameEditorModal;
...
fn find_preview_txn(state: AppState, rule: &NicknameRule) -> String {
    ...
    // Fallback when no transaction matches the rule's regex:
    rule.regex.clone()
}

#[component]
pub fn NicknameRulesModal() -> impl IntoView {
    ...
    let initial_rules = state.nickname_rules.get_untracked();
    ...
    let on_cancel = move |_| {
        state.is_nickname_manager_open.set(false);
    };
    ...
    let on_save = move |_| {
        ...
        spawn_local(async move {
            state.log(format!(
                "[Nicknames] saving {} nickname rules to persistent config",
                draft.len()
            ));
            if let Err(e) = crate::ipc::save_nickname_rules(state, draft.clone()).await {
                state.log(format!("[Nicknames] failed to save nickname rules list: {e}"));
            } else {
                state.nickname_rules.set(draft);
    ...
    let key_handle = window_event_listener(leptos::ev::keydown, move |ev| {
        if !state.is_nickname_manager_open.get_untracked() {
            return;
        }
    ...
    view! {
        <div class="modal-overlay">
            <div class="modal-container rules-modal-container" style=drag_style on:click=|ev| ev.stop_propagation()>
                <h2 on:mousedown=on_drag_start>"Manage Vendor Nickname Rules"</h2>

                <div class="rules-list-container">
                    <div class="rules-list-header">
                        <div class="col-pattern">"Pattern"</div>
                        <div class="col-pane">"Nickname"</div>
                    ...
                    <div class="rules-list-rows">
                        {move || {
                            ...
                            if draft.is_empty() {
                                view! { <div class="rules-empty-state">"No nickname rules defined"</div> }.into_any()
                            } else {
                                draft.into_iter().enumerate().map(|(i, rule)| {
                                    ...
                                    view! {
                                        ...
                                        >
                                            <div class="col-pattern">{rule_clone.regex.clone()}</div>
                                            <div class="col-pane">{rule_clone.nickname.clone()}</div>
        ...
        {move || editing_rule_index.get().map(|idx| {
            ...
            let preview_vendor = find_preview_txn(state, &rule_to_edit);

            view! {
                <NicknameEditorModal
                    preview_vendor=preview_vendor
                    initial_regex=rule_to_edit.regex.clone()
                    initial_nickname=rule_to_edit.nickname.clone()
                    on_save=move |updated_rule: NicknameRule| {
                        rules_draft.update(|rules| {
                            if idx < rules.len() {
